	...
	render: function (argument) {
		...
		App.contracts.AdminContract.deployed()
			...
			.then(function (retailerCount) {
				...
				var producerOption =
					"<option value='" + null + "'disabled selected >" + "Select Retailer" + "</ option>";
		...
		if (rAddress != null && pType != null && productlistSelect != null && quantity != "") {
			if (quantity > ConApp.QuantityAvailable || quantity == 0) {
				...
				App.contracts.NodeContract.deployed().then(function (instance) {
					pInstance = instance;
					instance
						.getCostForConsumer(rAddress, productname, pType, quantity)
						.then(function (amount) {
							...
							if (proceed) {
								pInstance
									.soldToConsumer(rAddress, productname, pType, quantity, {
										from: App.account,
