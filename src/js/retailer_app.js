	...
	render: function (argument) {
		...
		App.contracts.AdminContract.deployed()
			...
			.then(function (producerCount) {
				...
				var producerOption =
					"<option value='" + null + "'disabled selected >" + "Select Producer" + "</ option>";
		...
		if (pAddress != null && pType != null && productlistSelect != null && quantity != "") {
			if (quantity > RecApp.QuantityAvailable || quantity == 0) {
				...
				App.contracts.NodeContract.deployed().then(function (instance) {
					pInstance = instance;
					pInstance
						.getCostForRetailer(pAddress, productname, pType, quantity)
						.then(function (amount) {
							var proceed = confirm(
								"Total Cost of product(s):" + amount + " ethers\nPress ok to continue"
							);
							if (proceed) {
								pInstance
									.soldToRetailer(pAddress, productname, pType, quantity, {
										from: App.account,
					...
					if (count.c[0] < productid) {
						...
						pInstance.ProductList(productid).then(function (singleProduct) {
							...
							if (
								...
								pInstance
									.addReturnProductToRetailer(productid, singleProduct[2], percent, {
										from: App.account,
										value: web3.toWei(amount, "ether"),
									})
									.then(function (receipt) {
